...
**최종 업데이트**: 2024-11-24 (문서 475개, 발표자료 추가, Dependencies 업데이트)
...
**목적**: 정확한 기안자별 문서 검색 및 깨끗한 텍스트 미리보기 제공

**문제점**:
- "2025년 남준수 문서" 검색 시 다른 기안자(최새름 등) 문서도 함께 반환됨
- 검색 결과 snippet에 `[페이지 1]`, `[OCR 추출 텍스트]` 같은 메타데이터 마커 표시

**해결 방법**:

1. **마커 필터링** (display-level)
   - 위치: `app/rag/pipeline.py` lines 1107-1116
   - 정규식으로 `[페이지 X]`, `[OCR ...]` 패턴 제거
   - 깨끗한 텍스트 미리보기 제공

2. **기안자 필터링** (DB-level)
   - 위치: `app/rag/pipeline.py` lines 1009-1109
   - 쿼리에서 기안자명 자동 추출 (8명: 남준수, 최새름, 유인혁, 이의주, 강병규, 박연수, 이호영, 이승헌)
   - SQL WHERE 절에 `drafter = ?` 조건 추가
   - 매칭되지 않는 문서는 결과에서 제외

**테스트 결과**:
```
쿼리: "2025년 남준수 문서좀 찾아줘"
Before: 8개 문서 (남준수 5개 + 작성자 미상 3개)
After:  5개 문서 (남준수만, 100% 정확도)

✅ 다른 기안자 문서 제외됨
✅ 마커 없는 깨끗한 snippet 표시
```
...
### cache_manager.py 정리 (20줄 삭제)

**목적**: 사용되지 않는 import, 상수, 함수 제거로 코드 간결성 확보

**변경 사항**:
- `app/rag/cache_manager.py`: 163줄 → 143줄 (12% 감소)

**제거된 코드**:
1. **사용되지 않는 import** (2줄) - `hashlib`, `json` 모듈
2. **FREQUENT_QUERIES 상수** (12줄) - 사용되지 않는 샘플 쿼리 목록
3. **preload_frequent_queries() 함수** (6줄) - 구현되지 않은 캐시 사전 로드 함수

**효과**:
- 불필요한 의존성 제거 (hashlib, json)
- 미구현 기능 코드 제거로 혼란 방지
- 캐시 관리 코드 간결성 확보
...
### 원본 문서
- **위치**: `docs/year_*/`
- **파일 수**: 475개 PDF (2014-2025)
- **구성**: 방송 장비 결재 문서, 시설 관리 문서, 구매 검토서 등
- **연도 분포**: 2025년 19개, 2024년 20개, 2023년 33개 등
...
- **위치**: `metadata.db`
- **크기**: 3.1MB
- **모드**: SQLite WAL (Write-Ahead Logging)
- **내용**: 475개 문서 메타데이터, 청크 정보
...
- 문서 인덱싱: 완료 (475개 문서, 2014-2025)
- 텍스트 품질: 양호 (OCR 처리 완료)
- 검색 인덱스: 최신 (2024-11-24)
- URL 노이즈: 제거 완료
- 서비스 구성: FastAPI (7860) + Streamlit (8501)
- Dependencies: 업데이트 완료 (fastapi, uvicorn, PyYAML 추가)
