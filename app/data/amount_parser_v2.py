...
def _parse_eok_man(text: str) -> list[tuple[int, int, int, str]]:
    ...
    for m in RE_EOK_MAN.finditer(text):
        # 콤마/공백 제거, 숫자와 마침표만 유지 (OCR 노이즈 대응)
        eok_str = re.sub(r"[^\d\.]", "", m.group(1))
        man_str = re.sub(r"[^\d\.]", "", m.group(2) or "0")
        try:
            eok = float(eok_str) if eok_str else 0
            man = float(man_str) if man_str else 0
    ...
    for m in RE_MAN_ONLY.finditer(text):
        ...
        # 콤마/공백 제거, 숫자와 마침표만 유지
        man_str = re.sub(r"[^\d\.]", "", m.group(1))
        try:
            man = float(man_str) if man_str else 0
