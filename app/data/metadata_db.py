...
v2.1 변경사항 (2025-11-24):
- sqlite_helpers.connect_metadata() 통합으로 WAL 모드 일관성 확보
- thread-local 연결 관리 유지 (기존 호환성)
"""
...
class MetadataDB:
    ...
    def _new_conn(self) -> sqlite3.Connection:
        """새 SQLite 연결 생성 (스레드별) - v2.1: sqlite_helpers 통합"""
        # connect_metadata()는 이미 WAL, busy_timeout, foreign_keys 등 설정
        conn = connect_metadata(db_path=self._db_path, enable_row_factory=True)
    ...
    def init_database(self) -> None:
        ...
        logger.info(f"MetadataDB 초기화: {self._db_path}, journal_mode={journal_mode}")
    ...
    def get_page_text(self, doc_id: str, page: int) -> Optional[str]:
        ...
        try:
            ...
            pdf_path = self._docs_root / row["path"]
    ...
    def update_text_preview(self, path: str, text_preview: str) -> None:
        """텍스트 미리보기 업데이트"""
        with self._cursor() as cur:
            cur.execute(
