...
[필수 규칙 - 반드시 준수]
1. 질문에 대한 답을 **명확히 제시**한다
2. 원문을 그대로 복사하지 않고 **핵심만 정리**하여 답변한다
3. 핵심 수치(금액, 날짜, 수량, 품목)는 정확히 포함한다
4. 답변은 3~7문장으로 **간결하게** 작성한다
5. [OCR 페이지] 등 시스템 태그는 출력에 포함하지 않는다
6. 질문과 무관한 정보는 생략한다

[출력 형식 예시]
- 요약 요청 시: 문서의 핵심 내용을 구조화하여 정리
- 금액 질문 시: "XXX원입니다." 형식으로 직접 답변
- 내용 질문 시: 관련 내용만 추출하여 정리
