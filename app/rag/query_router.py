...
class QueryRouter:
    ...
    # 2025-12-30: "대수/종류/개수" 등 목록/집계 질의 패턴 추가
    LIST_INTENT_PATTERN = re.compile(
        r"("
        r"(\d{4}년?|[가-힣]{2,4}(가|이)?|모든|모두|전체|전부|all).*(찾아|검색|리스트|목록|보여|문서)|"  # 기존 패턴
        r"(대수|종류|개수|수량|갯수|목록|리스트).*(알려|뭐야|어떻게|무엇)"  # 목록/집계 질의 패턴 (NEW)
        r")",
        re.IGNORECASE,
    )

    # 정보 질의 패턴 (구체적 정보 + 질문형) - QA 모드로 라우팅
    # 예: "DVR IP 알려줘", "비용 얼마야", "현황 어떻게 돼", "IP 배정 현황 보여줘"
    # 2025-12-12: "보여줘" 추가 - 정보성 키워드(IP/현황/배정/설명 등) + "보여줘"는 QA 모드
    QA_QUESTION_PATTERN = re.compile(
        r"(IP|주소|비용|금액|날짜|내용|정보|설명|현황|사양|스펙|가격|수량|규격|모델|상태|결과|배정).*(알려|뭐야|뭔가요|어떻게|무엇|어떠|어때|보여)",
        re.IGNORECASE,
    )
    ...
    # "2025년 문서들은 어떤 내용이야?", "2024년 문서는 어떤 내용이야?" - 단수/복수 모두 지원
    YEAR_SUMMARY_PATTERN = re.compile(
        r"("
        # 패턴1: "2024년 문서 다 알려줘" (다/전부/전체/모두/모든 + 동사)
        r"(\d{4})\s*년?\s*(문서|기안서|검토서)?들?\s*(다|전부|전체|모두|모든).*"
        r"(알려|요약|정리|보여|읽|뭐야|뭔가요|어때|어떻게|있어)"
        r"|"
        # 패턴2: "2025년 문서(들)은/는 어떤 내용이야?" (연도 + 문서 + 은/는 + 내용 질문)
        # 단수("문서는"), 복수("문서들은") 모두 매칭
        r"(\d{4})\s*년?\s*(문서|기안서|검토서)들?(은|는)\s*.*(내용|뭐|어떤|무슨)"
        r"|"
        # 패턴3: "다/전부/전체 + 연도" 순서
    ...
    SEARCH_INTENT_PATTERN = re.compile(
        ...
        r"(있어\??|있나요|있는지)|"     # "있어?", "있나요"
        r"(몇\s*개|개수|총|count|number)|"  # "몇개", "개수", "총"
        r"(언제|이력|최근|최신|히스토리|history|"  # 시간순 쿼리 (2025-12-26 확장)
        r"작년|올해|금년|전년도|지난해|"  # 연도 관련
        r"내역|과거|예전|이전)|"  # 이력/시간 관련
