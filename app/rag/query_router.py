...
class QueryRouter:
    ...
    def __init__(self, config_path: str = "config/document_processing.yaml"):
        ...
        self.qa_keywords = self.config.get('mode_routing', {}).get('qa_intent_keywords', [])
        self.preview_keywords = self.config.get('mode_routing', {}).get('preview_only_keywords', [])
        self.filename_pattern = self.config.get('mode_routing', {}).get('filename_pattern', r'\S+\.pdf')

        logger.info(f"📋 모드 라우터 초기화: QA 키워드 {len(self.qa_keywords)}개, 미리보기 키워드 {len(self.preview_keywords)}개")
    ...
    def classify_mode(self, query: str) -> QueryMode:
        ...
        # 3. 미리보기 전용 키워드 체크
        has_preview_intent = any(keyword in query_lower for keyword in self.preview_keywords)

        # 결정 로직
        if has_qa_intent:
            # Q&A 의도가 있으면 파일명이 있어도 Q&A 모드
            logger.info(f"🎯 모드 결정: Q&A (의도 키워드 감지)")
            return QueryMode.QA

        elif has_filename and has_preview_intent:
            # 파일명 + 미리보기 전용 키워드 → 미리보기 모드
            logger.info(f"🎯 모드 결정: PREVIEW (파일명 + 미리보기 키워드)")
            return QueryMode.PREVIEW

        elif has_filename and not has_qa_intent:
            # 파일명만 있고 Q&A 의도 없음 → 미리보기 모드
            logger.info(f"🎯 모드 결정: PREVIEW (파일명만 존재)")
            return QueryMode.PREVIEW

        else:
            # 기본: Q&A 모드
            logger.info(f"🎯 모드 결정: Q&A (기본)")
            return QueryMode.QA
    ...
    def get_routing_reason(self, query: str) -> str:
        ...
        detected_preview_keywords = [kw for kw in self.preview_keywords if kw in query_lower]
        ...
        if has_preview_intent:
            reason_parts.append(f"preview_keywords({','.join(detected_preview_keywords)})")
