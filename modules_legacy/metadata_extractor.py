...
# 강화된 금액 파서 임포트
from modules.amount_parser_v2 import extract_amount_candidates, select_document_amount, validate_amount
...
class MetadataExtractor:
    ...
    def __init__(self):
        ...
        self._date_patterns = [
            ...
            (re.compile(r"(\d{4})[.\-\/](\d{1,2})(?![.\-\/\d])"), "ym"),  # YYYY.MM 형식 (일자 누락)
        ...
        self._dept_patterns = [
            # Channel A 특화 조직
            (re.compile(r"(방송기술팀|보도기술관리파트|보도기술관|디지털미디어랩|NQC운영실|송출실|멀티스튜디오)"), "channel_a"),
            # 일반 기술부서
            ...
            # 명시적 부서 표기
            (re.compile(r"부서\s*:\s*([가-힣A-Za-z]+(?:부|팀|실|과|센터|본부|파트))"), "general"),
        ...
        self._known_companies = [
            ...
            "arri", "red", "blackmagic", "dji", "atomos", "hanwha", "techwin", "hanwha vision", "tvlogic", "ross",
            "grass valley", "imagine", "sony broadcast", "ikegami", "hitachi",
            # 국내 협력사/벤더
            "영도비엔씨", "미러클랩", "디지털홍일", "아이티엔에스", "씨넥스", "엔터프라이즈", "브로드밴드", "비즈텔",
            "한맥", "동양시스템즈", "대림정보통신", "케이씨디", "지디에스", "에이엠티",
            # 방송사
            "kbs", "mbc", "sbs", "ebs", "jtbc", "channel a", "채널a", "채널에이", "ytn", "tvn"
        ]
        # (주) 변형
    ...
    def _extract_amounts(self, text: str) -> Dict[str, Any]:
        ...
        if is_valid and validated_amount:
            ...
            if candidates:
                ...
                # 문서 상단→하단 순서로 정렬 (사용자 직관에 부합)
                res["all_amounts"].sort(key=lambda x: x["position"])
