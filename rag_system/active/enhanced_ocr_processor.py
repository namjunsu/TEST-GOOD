...
def _ocr_worker(args: Tuple[str, Dict]) -> Tuple[str, str, Dict]:
    ...
    Args:
        args: (pdf_path, options) 튜플
            - pdf_path: PDF 파일 경로
            - options: 처리 옵션 (cache_dir 등)

    Returns:
        (pdf_path, text, metadata) 튜플
    """
    pdf_path, options = args
...
class EnhancedOCRProcessor:
    ...
    def _compile_patterns(self):
        ...
        # 한글 병합 패턴 컴파일 (2글자, 3글자 모두 처리)
        self.korean_merge_pattern_3 = re.compile(r"([가-힣])\s+([가-힣])\s+([가-힣])")
        self.korean_merge_pattern_2 = re.compile(r"([가-힣])\s+([가-힣])")
        ...
        try:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for page in pdf.pages[:3]:
                    ...
                    if text and len(text.strip()) > self.TEXT_LAYER_MIN_CHARS:
                        return True
            return False
        except Exception as e:
    ...
    def process_pdf_with_ocr(self, pdf_path: str, page_num: Optional[int] = None, lang: str = "kor+eng") -> Dict:
        ...
        try:
            ...
            if result["ok"]:
                ...
                logger.warning("[OCR] OCR 실패: 추출된 텍스트 없음")
    ...
    def _merge_korean_words(self, text: str) -> str:
        """분리된 한글 단어 병합 (2글자, 3글자 모두 처리)"""
        # 3글자 병합 먼저 처리
        text = self.korean_merge_pattern_3.sub(
        ...
        # 2글자 병합 처리
        text = self.korean_merge_pattern_2.sub(
