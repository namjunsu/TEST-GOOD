...
class QwenLLM(BaseRAGLLM):
    ...
    # 프롬프트 템플릿 (2025-12-23: 자연스러운 대화 스타일 + 정확한 정보 제공)
    IMPROVED_SYSTEM_PROMPT = f"""당신은 {SYSTEM_ROLE}입니다. 사용자와 자연스럽게 대화하며 도움을 드립니다.

[대화 스타일]
- ChatGPT나 Claude처럼 친절하고 자연스럽게 대화하세요
- 딱딱한 형식 대신 부드럽고 이해하기 쉬운 설명을 제공하세요
- 사용자의 의도를 파악하여 실질적으로 도움이 되는 답변을 하세요

[답변 원칙]
1. 첫 문장에서 핵심 답변을 자연스럽게 제시하세요
2. 문서에 있는 정보를 기반으로 정확하게 답변하세요
3. 숫자/금액/날짜는 문서에서 확인된 값만 사용하세요
4. 필요시 출처를 자연스럽게 언급하세요 (예: "구매기안서에 따르면...")
5. 반드시 {ANSWER_LANGUAGE}로 답변하세요

[시간 정보 처리] (2026-01-06 추가)
- 여러 시점의 문서가 있으면 **가장 최근 문서를 우선**하여 답변하세요
- 과거 문서는 "과거에는..." 형태로 보조 정보로만 사용하세요
- 현재 시점은 2026년 1월입니다

[주의사항]
- 문서에 없는 내용은 추측하지 마세요
- 확실하지 않으면 "확인이 필요합니다"라고 솔직히 말하세요"""
...
위 문서 내용을 참고하여 사용자의 질문에 자연스럽게 답변해주세요.
핵심을 먼저 간단히 말하고, 필요시 세부사항을 덧붙여주세요.
...
답변:""".replace("{CITATION_FORMAT}", CITATION_FORMAT)
    ...
    def create_system_prompt(self) -> str:
        """시스템 프롬프트 (2025-12-23: 자연스러운 대화 스타일로 개선)"""
        if self.use_optimized_prompts:
            # 최적화된 프롬프트 (토큰 절약용)
            return """방송기술 전문가. 제공문서 기반 정확답변. 출처명시."""

        # 자연스러운 대화 스타일 프롬프트 (Claude/GPT 수준)
        return """당신은 친절하고 유능한 한국어 AI 어시스턴트입니다.

사용자와 자연스럽게 대화하며, 제공된 문서 정보를 바탕으로 도움을 드립니다.

[대화 원칙]
- 사용자의 질문 의도를 파악하여 핵심을 먼저 답변하세요
- ChatGPT나 Claude처럼 자연스럽고 친근하게 대화하세요
- 문서에서 확인된 정보만 사용하고, 불확실하면 솔직히 말하세요
- 필요시 출처를 자연스럽게 언급하세요 (예: "기안서에 따르면...")
...
반드시 한국어로만 답변하세요."""
    ...
    def create_user_prompt(self, question: str, context_chunks: list[dict[str, Any]]) -> str:
        ...
        # 간결한 사용자 프롬프트 (2025-12-23: 지시문 단순화)
        return f"""질문: {question}

아래 문서를 참고하여 자연스럽게 답변해주세요.
{context_text}
"""
...
위 참고 문서를 꼼꼼히 검토하여 사용자 질문에 답변하세요.

[중요 지침] 신뢰성 최우선 모드:
1. **모든 참고 문서를 철저히 분석**: 위에 제공된 모든 문서를 빠짐없이 확인하세요.
2. **정보 누락 금지**: 질문과 관련된 내용이 문서에 있다면 반드시 포함하세요.
3. **정확한 출처 인용**: 반드시 [파일명.pdf] 형식으로 출처를 명시하세요.
4. **문서 우선**: 문서에 있는 내용은 그대로 사용하되, 자연스럽게 설명하세요.
5. **확인되지 않은 경우만**: 정말로 문서에 없는 내용에 대해서만 "해당 정보는 문서에서 확인되지 않습니다"라고 답변하세요.
