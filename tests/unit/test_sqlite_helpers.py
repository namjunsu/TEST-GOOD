...
class TestMetadataTxn:
    """metadata_txn 컨텍스트 매니저 테스트"""

    def test_transaction_commit_on_success(self, tmp_path, monkeypatch):
        """정상 완료 시 자동 커밋"""
        from app.utils.sqlite_helpers import metadata_txn

        db_path = tmp_path / "test_txn_commit.db"
        monkeypatch.setenv("METADB_PATH", str(db_path))

        # 트랜잭션 내에서 테이블 생성 및 데이터 삽입
        with metadata_txn(mode="IMMEDIATE", db_path=str(db_path)) as conn:
            conn.execute("CREATE TABLE test (id INTEGER, value TEXT);")
            conn.execute("INSERT INTO test VALUES (1, 'committed');")

        # 새 연결에서 데이터 확인 (커밋되었는지)
        verify_conn = connect_metadata(str(db_path))
        cursor = verify_conn.execute("SELECT value FROM test WHERE id=1;")
        result = cursor.fetchone()[0]
        verify_conn.close()
    ...
    def test_transaction_rollback_on_error(self, tmp_path, monkeypatch):
        ...
        db_path = tmp_path / "test_txn_rollback.db"
        monkeypatch.setenv("METADB_PATH", str(db_path))

        # 초기 테이블 생성
        init_conn = connect_metadata(str(db_path))
        init_conn.execute("CREATE TABLE test (id INTEGER, value TEXT);")
        init_conn.execute("INSERT INTO test VALUES (1, 'original');")
        init_conn.close()

        # 트랜잭션 내에서 예외 발생
        try:
            with metadata_txn(mode="IMMEDIATE", db_path=str(db_path)) as conn:
                conn.execute("UPDATE test SET value='changed' WHERE id=1;")
                raise ValueError("Simulated error")
        except ValueError:
            pass  # 예상된 예외

        # 새 연결에서 데이터 확인 (롤백되었는지)
        verify_conn = connect_metadata(str(db_path))
        cursor = verify_conn.execute("SELECT value FROM test WHERE id=1;")
        result = cursor.fetchone()[0]
        verify_conn.close()

        assert result == "original"  # 변경되지 않았어야 함

    def test_transaction_mode_immediate(self, tmp_path):
        """IMMEDIATE 모드 트랜잭션"""
        from app.utils.sqlite_helpers import metadata_txn

        db_path = tmp_path / "test_txn_immediate.db"
    ...
    def test_transaction_mode_exclusive(self, tmp_path):
        """EXCLUSIVE 모드 트랜잭션"""
        from app.utils.sqlite_helpers import metadata_txn

        db_path = tmp_path / "test_txn_exclusive.db"

        with metadata_txn(mode="EXCLUSIVE", db_path=str(db_path)) as conn:
            conn.execute("CREATE TABLE test (id INTEGER);")
            for i in range(100):
                conn.execute(f"INSERT INTO test VALUES ({i});")
