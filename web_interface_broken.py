...
@st.cache_resource
def initialize_rag_system(version="v2.1"):  # 버전 변경시 캐시 무효화
    """RAG 시스템 초기화 (한 번만 실행)"""
    import importlib
    # 모듈 강제 리로드
    import quick_fix_rag
    importlib.reload(quick_fix_rag)
    from quick_fix_rag import QuickFixRAG
    return QuickFixRAG()
...
def main():
    ...
    if 'rag' not in st.session_state:
        ...
        with loading_container.container():
            ...
            # 실제 RAG 시스템 초기화
            st.session_state.rag = initialize_rag_system("v2.1")
        ...
        if not df.empty:
            ...
            with tab2:
                ...
                if not df.empty and 'year' in df.columns:
                    ...
                    # 선택된 연도 추출 (연도없음 처리)
                    if selected_year_str == "연도없음" or "연도없음" in selected_year_str:
                        selected_year = 0
